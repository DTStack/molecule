...
function Menu(props) {
    ...
    var content = react_1.cloneReactChildren(children, { onClick: onClick });
    ...
    if (data.length > 0) {
        var renderMenusByData_1 = function (menus) {
            return menus.map(function (item) {
                var handleClick = utils_1.mergeFunctions(onClick, item.onClick);
                if (item.data && item.data.length > 0) {
                    return (React.createElement(subMenu_1.SubMenu, __assign({ key: item.id, mode: mode }, item, { onClick: handleClick }), renderMenusByData_1(item.data)));
                }
                return (React.createElement(menuItem_1.MenuItem, __assign({ key: item.id, onClick: handleClick }, item), item.name));
            });
        };
